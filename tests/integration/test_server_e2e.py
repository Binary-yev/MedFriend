...
A2A_RPC_URL = BASE_URL + "/a2a/care_navigator/"
...
def start_server() -> subprocess.Popen[str]:
    ...
    command = [
        ...
        "-m",
        "uvicorn",
        "care_navigator.fast_api_app:app",
        "--host",
        "0.0.0.0",
        "--port",
        "8000",
    ...
    env.setdefault("GOOGLE_MAPS_API_KEY", "dummy")
...
def test_adk_run_sse(server_fixture: subprocess.Popen[str]) -> None:
    ...
    session_response = requests.post(
        f"{BASE_URL}/apps/care_navigator/users/{user_id}/sessions",
        headers=HEADERS,
        json=session_data,
        timeout=60,
    )
