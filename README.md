...
Built for **Google & Kaggle's 5‑Day AI Agents Intensive** — see [CAPSTONE.md](CAPSTONE.md) for how it maps to the capstone requirements.
...
**Approval gates on every real‑world action — and for the irreversible ones, enforced in code.** Submitting an appeal, sending an email, placing a phone call, and booking an appointment all **stop and wait for explicit patient approval**. The agent will not contact the insurer or the office at document‑intake time, and it never announces an approval a counterparty didn't actually return.

Two of those actions are irreversible and leave the process: `send_mail` performs a real Gmail send, and `place_complaint_call` makes a real phone ring and costs real money. For those two the gate is **not** an instruction the model may or may not follow — both are wrapped in ADK's `FunctionTool(func=…, require_confirmation=True)`, so ADK suspends the call, emits an `adk_request_confirmation` request, and `FunctionTool` returns an error *before ever invoking the function body*. Execution resumes only when a `FunctionResponse` carrying `{"confirmed": true}` arrives out‑of‑band — a structured signal the model cannot synthesize for itself. In the ADK web playground this surfaces as an inline confirmation card with a **Confirmed** toggle and a **Submit** button; typing "yes" in the chat box does *not* satisfy it, which is precisely the point. A jailbroken model, an injection that survived Layers 1–2, or a judge that failed open still cannot reach the network call.

The other two gates stay prompt‑level, deliberately: appeal submission and booking route to `insurance_reviewer` and `provider_office`, which are bare `LlmAgent`s with **no tools of their own** — they role‑play a counterparty reply and nothing leaves the process, so there is no irreversible effect to gate. `tests/unit/test_approval_gates.py` asserts both halves of that, including a test that fails loudly if either sub‑agent ever acquires a tool and invalidates the reasoning.
...
**Secrets hygiene.** No API keys or passwords are committed — verified: **no keys or tokens in the working tree or git history**. Keys come from environment variables (or git‑ignored file paths — `credentials.json`, `token.json`, `api_key.txt`, `*.pem`, and `*-key.json` are all in `.gitignore`); Gmail uses the patient's own OAuth token, refreshed silently and never committed.
...
├── tests/
│   ├── unit/                       # Fast, dependency-free tests: tamper-defense, Layer-1 filter, Layer-3 judge, approval gates
...
- `tests/unit/` — fast, dependency‑free tests of the security‑critical logic, no model or network required: `test_case_tools.py` covers the tamper‑defense state machine (save vs. quarantine, quarantine isolation, discard, plan‑fact lookups), `test_security.py` covers the deterministic Layer‑1 filter (PII scrubbing, injection detection, the email pre‑filter, and the `before_model_callback`), `test_judge_plugin.py` covers the Layer‑3 runtime judge with its sub‑agent stubbed out (both wired hooks on safe / unsafe / no‑verdict, fail‑closed on tool calls vs. fail‑open on responses, and the hooks left deliberately unwired), and `test_approval_gates.py` covers the code‑enforced confirmation on the two irreversible tools (an unconfirmed call never reaches the function body, a rejected one is refused, and the gate list stays exactly `send_mail` + `place_complaint_call`).
